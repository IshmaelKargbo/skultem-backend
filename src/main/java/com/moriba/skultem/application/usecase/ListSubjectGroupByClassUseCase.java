...
public class ListSubjectGroupByClassUseCase {
    private final SubjectGroupRepository repo;

    public Page<SubjectGroupDTO> execute(String schoolId, String classId, int page, int size) {
        Pageable pageable = Pageable.unpaged();
        if (size > 0) {
            pageable = PageRequest.of(page, size);
        }
        return repo.findByClassAndSchool(classId, schoolId, pageable).map(SubjectGroupMapper::toDTO);
    }
}
