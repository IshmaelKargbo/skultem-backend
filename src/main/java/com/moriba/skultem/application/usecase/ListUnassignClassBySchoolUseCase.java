...
public class ListUnassignClassBySchoolUseCase {
    ...
    public Page<ClassSessionDTO> execute(String school, int page, int size) {
        ...
        Pageable pageable = Pageable.unpaged(Sort.by(Sort.Order.asc("clazz.levelOrder")));
        ...
        return repo.findUnassignedBySchoolAndAcademicYear(school, academicYear.getId(), pageable)
                .map((e) -> {
                    ...
                    return new ClassSessionDTO(e.getId(), clazzName, classId, teacherName, teacherId, count, streamName,
                            streamId, sectionName, sectionId, classLevel, grade, null, false);
                });
    }
}
