...
public class AssignSubjectsToStreamUseCase {
    ...
    public void execute(String schoolId, String streamId, List<SubjectAssignment> assignments) {
        ...
        var existingMap = existing.stream()
                .collect(Collectors.toMap(
                        ...
                        ss -> ss,
                        (a, b) -> { throw new RuleException("Duplicate stream subject records exist. Clean up first."); }
                ));
        ...
        for (var assignment : assignments) {
            ...
            if (!subject.getSchoolId().equals(schoolId)) {
                throw new RuleException("Subject '" + subject.getName() + "' does not belong to this school.");
            }
        ...
        for (var assignment : assignments) {
            ...
            if (assignment.subjectGroupId() != null && !assignment.subjectGroupId().isBlank()) {
                ...
                if (group == null) {
                    throw new RuleException("Invalid subject group: " + assignment.subjectGroupId());
                }
            }
            ...
            if (existingMap.containsKey(subject.getId())) {
                ...
                var record = StreamSubject.create(UUID.randomUUID().toString(), schoolId, stream, subject, group, assignment.mandatory());
                repo.save(record);
            }
        }
        ...
        for (var ss : existing) {
            if (!incomingSubjectIds.contains(ss.getSubject().getId())) {
                ...
                var enrollments = enrollmentRepo
                        .findAllByStreamIdAndAcademicYearIdAndSchoolId(streamId, academicYear.getId(), schoolId);
        ...
        syncAssessmentsForEnrolledStudents(schoolId, streamId, academicYear.getId(), assignments, incomingSubjects, removedSubjectIds);
    }
    ...
    private void syncAssessmentsForEnrolledStudents(
        ...
        if (requiredSubjectIds.isEmpty() && removedSubjectIds.isEmpty()) return;
        ...
        for (Enrollment enrollment : enrollments) {
            ...
            for (String subjectId : requiredSubjectIds) {
                ...
                enrollmentSubjectRepo.save(EnrollmentSubject.create(
                        enrollmentSubjectId, schoolId, enrollment, subject, enrollment.getStudent()));
            }
