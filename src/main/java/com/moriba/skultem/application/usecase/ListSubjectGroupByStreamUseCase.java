...
public class ListSubjectGroupByStreamUseCase {
    private final SubjectGroupRepository repo;

    public Page<SubjectGroupDTO> execute(String schoolId, String streamId, int page, int size) {
        Pageable pageable = Pageable.unpaged();
        if (size > 0) {
            pageable = PageRequest.of(page, size);
        }
        return repo.findByStreamIdAndSchoolId(streamId, schoolId, pageable).map(SubjectGroupMapper::toDTO);
    }
}
